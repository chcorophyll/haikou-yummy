...
### Phase 1：基础框架搭建 — [已完成 ✅]
- [x] 初始化 UniApp + Vue 3 项目，接入 UnoCSS
- [x] 搭建全局设计系统（颜色变量、通用 CSS）
- [x] 实现 `NavBar`、`RestaurantCard`、`Toast` 等基础组件
- [x] 配置后端 API 请求层（封装 `wx.request`）

### Phase 2：地图与核心功能 — [已完成 ✅]
- [x] 实现地图主页：腾讯地图集成、自定义标注、定位蓝点
- [x] 实现搜索栏 + 快捷菜系标签筛选
- [x] 实现 `BottomSheet` 弹窗展示餐厅信息
- [x] 实现餐厅列表页与详情页

### Phase 3：社交与体验打磨 — [已完成 ✅]
- [x] 实现收藏功能（Pinia + wx.Storage 持久化）
- [x] 实现"推荐宝藏店"提交表单
- [x] **深度架构优化 (Phase 2.5)**：解决详情页二次闪屏问题，实现原子化渲染锁与原生导航栏冲突修复。
- [x] **Tesla 视觉进化**：全面汉化并重构 `submit` 页面，实现完美的垂直居中与极简高对比度设计。
- [x] 微信开放平台配置、真机测试与验证
